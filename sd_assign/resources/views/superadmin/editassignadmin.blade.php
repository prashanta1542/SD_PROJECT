...
<div style="background-color:rgba(120,60,150,0.6); height:100vh;">
    ...
    <div class="container">
        ...
        @endif
        <form method="post" action="{{url('updatedepartment/'.$d->id)}}">
            @csrf

            <div class="form-group">
                <label for="name">Name of Department:</label>
                <input type="text" class="form-control" placeholder="Department name" value="{{$d->name}}" id="name" name="name">
            </div>
            <div class="form-group">
                <label for="aliases">Short Aliase</label>
                <input type="text" class="form-control" placeholder="Short form of department" value="{{$d->aliase}}" id="aliases" name="aliases">
            </div>
            <div class="form-group">
                <label for="admin">Assign Department Admin</label>
                <select class="form-control" id="admin" name="admin">
                    <option value="null">Choose department admin</option>
                    @foreach($e as $i)
                      @if($i->role == 'department-admin')
                          <option
                           value="{{$i->name}}"
                           {{($i->name)==($d->admin)?'selected':''}}
                          >
                          {{($i->name)}}
                          </option>
                      @endif
